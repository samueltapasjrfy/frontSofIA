...
export function AudiencesStats() {
    const { getTotalPendingQuery, getAudiencesQuery } = useAudiences();

    const { data: pendingData, isLoading, error } = getTotalPendingQuery;
    const { data: audiencesData, isLoading: isAudiencesLoading, error: audiencesError } = getAudiencesQuery;
    if (isLoading) {
        return (
